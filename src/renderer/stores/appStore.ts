 ...
 */
export function deduplicateProjects(projects: ClaudeProject[]): ClaudeProject[] {
...
export type NavigationView = 'claudemd' | 'files' | 'hooks' | 'rules' | 'skills' | 'agents' | 'commands';
    ...
    persist(
      (set) => ({
        ...
        removeFavorite: (path: string) =>
          set((state: FavoriteState) => ({
            favoritePaths: state.favoritePaths.filter((p: string) => p !== path),
          })),
      }),
      {
