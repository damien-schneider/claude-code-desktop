...
const MessageItem: React.FC<MessageItemProps> = ({ msg, index }) => {
...
const PermissionSelector: React.FC<PermissionSelectorProps> = ({
  ...
  return (
    <div className="mb-3 flex items-center gap-3">
      ...
      >
        <SelectTrigger className="h-8 text-sm" id="permission-mode">
...
export const ChatArea: React.FC = () => {
  ...
  const renderChatInterface = () => {
    const showCost = lastQueryCost !== null && !isStreaming && !isThinking;
        ...
        <div className="flex items-center justify-end gap-2 p-2">
          {isStreaming || isThinking ? (
            ...
            <PromptInputSubmit disabled={isSending}>
              {isSending && <Spinner className="mr-2 h-4 w-4 animate-spin" />}
            </PromptInputSubmit>
          )}
        </div>
      </PromptInput>
  ...
  const renderInput = () => {
    ...
    if (currentSessionId || activeProcessId) {
