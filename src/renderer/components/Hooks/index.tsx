...
console.log('${hookType} hook executed:', {
  ...
  const validateLifecycleMethods = (parsed: HookJson) => {
    const methods = [
      "onStart",
      "onSubmit",
      "onToolUse",
  ...
  const validateHookSyntax = (content: string) => {
    const parsed: HookJson = JSON.parse(content);
    ...
    validateLifecycleMethods(parsed);
  };
  ...
  const handleTestHook = () => {
    ...
    try {
      validateHookSyntax(selectedHookData.content || "{}");
      // biome-ignore lint/suspicious/noAlert: alert is used for validation result
      window.alert(
        "Hook validation passed! The hook structure and syntax are valid."
      );
    } catch (error) {
      // biome-ignore lint/suspicious/noAlert: alert is used for validation result
      window.alert(
