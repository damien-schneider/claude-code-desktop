...
import { showError, showSuccess, withPromise } from '@/renderer/lib/toast';

export type ClaudeItemType = 'skills' | 'commands' | 'agents' | 'rules' | 'hooks';
 ...
 */
export function useClaudeItems({ type, currentView: viewName }: UseClaudeItemsOptions) {
  ...
  const createItem = useCallback(async (name?: string) => {
    if (!activePath) {
      showError('Cannot create item', 'Please select a project or global settings first');
      return {
        success: false,
